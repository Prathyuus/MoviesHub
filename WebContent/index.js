...
function addToFav(i) {
	...
	if (count < 10) {
		...
		xmlhttp.open('GET', "http://localhost:8080/TatMovie/JsonParsing?"
				+ params, true);
		xmlhttp.send();
	} else {
...
function ShowFavourite() {
	...
	if (favvalidation == 1) {
		...
		xmlhttp.open('GET',
				"http://localhost:8080/TatMovie/RetriveFromJson?", true);
		xmlhttp.send();
	} else {
...
function removeFromFav(i) {
	...
	xmlhttp.open('GET', "http://localhost:8080/TatMovie/RemoveMovie?"
			+ params, true);
	xmlhttp.send();
}